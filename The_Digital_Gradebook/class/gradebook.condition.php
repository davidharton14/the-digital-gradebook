...
include_once '../class/gradebook.class.php'; 
...
if($_POST['action'] == "register-student"){
	...
	if($el->isStudentId($userid)){
		...
		$el->setStudent($userid, $fname, $lname, $email, $password, $date);
	}
}
...
if($_POST['action'] == "edit-student"){
	...
	if($confirm_pass !== $password){
		...
		$el->editStudent($userid, $fname, $lname, $email, $password, $date);
		//$el->setNewStudent('asdfghj', '123', '123', '123', '123', 'date');
		$el->setActivityLog($user, $action, $date);
	}
}
...
if($_GET['action'] == "select-student"){
	$id = $_GET['id'];
	list($id, $fname, $lname, $email, $password) = $el->getStudentID($id);
}
